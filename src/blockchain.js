...
class Blockchain {
    ...
    _addBlock(block) {
        ...
        return new Promise(async (resolve, reject) => {
           ...
           var hashed = SHA256(JSON.stringify(block)).toString();
           block.hash = hashed;
    ...
    async getStarsByWalletAddress (address) {
        ...
        let stars = [];
        return new Promise(async (resolve, reject) => {
            self.chain.forEach(async function (item, index) {
                var data = await item.getBData();
                if (data) {
        ...
        return new Promise(async (resolve, reject) => {
            self.chain.forEach(async function (item, index) {
                if (!await item.validate()) {
                    errorLog.push(`${item} isn't valid.`)
                }
                if (item.previousBlockHash != prevBlockHash) {
                    errorLog.push(`previous block hash: ${item.previousBlockHash} isn't valid for block with hash ${item.hash}`)
                }

                //set prev hash to this and move on.
                prevBlockHash = item.hash;
              });
